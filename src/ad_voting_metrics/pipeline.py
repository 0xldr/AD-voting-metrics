...
from .paths import RECONCILIATION_LOG_PATH, YAML_PATH
...
def run(period: MonthPeriod, *, rebuild: bool) -> None:
    """Pull data from on-chain + APIs and write the month's CSVs."""
    ...
    daily = _rank_daily_balances(delegation.get_delegate_list_sky(metrics, period, rebuild=rebuild))
    ...
    output_files = write_csvs(period, daily, metrics, poll_info, spell_info)
